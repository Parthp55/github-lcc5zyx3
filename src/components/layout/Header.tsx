      ...
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-xs md:text-sm text-orange-500 font-medium">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            <motion.a
              ...
              href={item.href}
              whileHover={{ y: -2 }}
              className={`font-medium text-cosmic-dark hover:text-orange-500 transition-colors ${
                item.name === 'Book a Reading' 
                  ? 'bg-orange-500 text-white px-4 py-2 rounded-full hover:bg-opacity-90 hover:text-white'
                  : ''
              }`}
            >
              {item.name}
        ...
        <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
          {navItems.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="font-medium text-cosmic-dark hover:text-orange-500 py-2 px-4 border-b border-cream-dark"
